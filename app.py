...
if "page" not in st.session_state:
    st.session_state["page"] = "Overview"  # Valeur par défaut
...
# Champs additionnels
df = df.drop([6529,6530,6531,6532])
df["Price"] = round(df["Total Sales"] / df["Units Sold"], 2)
df["Total Cost"] = round(df["Total Sales"] - df["Operating Profit"], 2)
df["Sexe"] = np.where(
...
selected_region = st.sidebar.multiselect("Sélectionnez la Région :", options=df["Region"].unique(), default=df["Region"].unique())
selected_retailer = st.sidebar.multiselect("Sélectionnez le Retailer :", options=df["Retailer"].unique(), default=df["Retailer"].unique())
selected_product = st.sidebar.multiselect("Sélectionnez le Produit :", options=df["Product"].unique(), default=df["Product"].unique())
selected_channel = st.sidebar.multiselect("Sélectionnez le Canal :", options=df["Sales Method"].unique(), default=df["Sales Method"].unique())
...
# Puis vos tabs
tabs = st.tabs(["🏠 Accueil", "📊 Overview", "🌍 Géographie", "👟 Produits",
                "🏬 fournisseurs", "🛒 Prix & Méthodes"])
...
with tabs[3]:
    ...
    with col1:
        ...
        fig_donut = px.pie(
            ...
            title="Répartition des Unités Vendues par Sexe",
            hole=0.6
        )
    ...
    with col2:
        ...
        prix_moyen = df_filtered.groupby(["Product", "Sexe"], as_index=False)["Price"].mean()
        prix_moyen = prix_moyen.sort_values("Price", ascending=True)
