...
for row in range(num_rows):
    for col in range(num_cols):
        ...
        size = 50#random.randint(60, 90)
        opacity = 0.12#random.uniform(0.1, 0.15)
        logos_html += f"""
        <div style="
            position: fixed;
            top: {top}%;
...
with tabs[2]:
    ...
    with col_map:
        ...
        fig_map.update_layout(
            title="Carte :Profit par État (USA)",
    ...
    with col_tables:
        if not df_sum.empty:
            ...
            st.dataframe(
                bottom2.style.format({"Profit": "${:,.0f}"}),
                use_container_width=True,
                height=120
            )
...
# =======================
    ...
    fig_time = px.line(
        ...
        title="Évolution du chiffre d'affaires par fournisseur"
...
with tabs[6]:
    ...
    st.markdown("""
- *Responsable Partenariats* : Renforcer les partenariats avec *WestGear*, *Sports Direct* et *Foot Locker*.  

- *Responsable Marketing* : Mettre en place des *campagnes ciblées pour les femmes* afin de dynamiser ce segment.  

- *Manager Régional* :
    - *Ajuster les prix* à la hausse dans les régions *South* et *West*.  
    - *Développer le canal In-store* dans le *South* et le *canal Outlet* dans le *West*.  

- *Responsable Commercial* : *Augmenter les prix sur le canal Online* pour générer un *chiffre d’affaires plus élevé*.  
    """)
    ...
    st.markdown(
        ...
        <p style='text-align:center; margin-top:20px;'>
            <img src='https://media.giphy.com/media/v1.Y2lkPWVjZjA1ZTQ3a2szZHFxaDN0NTByYm1kNXVxN283OGh6cng1YXBqNmo5eWRqNXNmZiZlcD12MV9naWZzX3JlbGF0ZWQmY3Q9Zw/UaowfCTS0INxkW48N4/giphy.gif' 
                 alt='animation' width='140' style='border-radius:10px;'>
        </p>
        """,
        unsafe_allow_html=True
    )
